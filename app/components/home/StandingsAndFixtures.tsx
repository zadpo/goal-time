...
export default function StandingsAndFixtures({
  ...
  return (
    <div className="flex flex-col w-full max-w-7xl  lg:flex-row">
          ...
          <div className="w-full flex flex-col justify-center items-center">
            ...
            <div className="flex justify-center w-full">
              {menuItems.map((a, i) => (
                <button
                  key={i}
                  className={`w-full p-4 g md:text-base
                                        text-xs font-bold
                                        ${
                                          i === activeTab ? "text-neutral-100" : "text-gray-700 bg-black/70"
                                        }`}
                    ...
                    {responseData.league.standings[0].map((team, j) => (
                      <Link
                        ...
                        className={`flex w-full p-1 hover:bg-red-800/50
                                                        ${j % 2 === 0 ? "bg-black/40" : ""}`}
                      >
                        <div className="w-1/12 flex px-2 justify-center items-center">{j + 1}</div>
                        <div className="w-3/12 flex text-xs items-center">{team.team.name}</div>
                ...
                return (
                  ...
                  filteredFixtures.map((league, j) => {
                    if (league.name === leagueName) {
                      return <FixturesByLeague fixturesData={league.fixtures} key={league.name + j} />;
                    }
                  })
                );
              })}

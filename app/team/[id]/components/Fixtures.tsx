...
export default function Fixtures({ fixturesByTeamId, teamId }: PageProps) {
  ...
  const nextItem = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === firstItemsFixturesFuture.length - 1 ? firstItemsFixturesFuture.length - 1 : prevIndex + 1
    );
  };
    ...
    <div className="flex flex-col w-full justify-center items-center text-neutral-100">
      <div className="flex flex-col w-full justify-center items-center">
        ...
        <div className="flex items-center justify-center relative overflow-hidden w-full">
          <button className="absolute left-0 top-1/2 transform -translate-y-1/2 p-2 z-10" onClick={prevItem}>
                  ...
                  <div className="flex w-full justify-between items-center h-2/6 md:text-2xl">
                    <div className="flex flex-col justify-center items-center">
                      {fixture.score.penalty.home !== null ? (
                        ...
                        <div className="text-sm">(et. ) {fixture.score.extratime.home}</div>
                      ) : null}
                    </div>
                    <div>-</div>
                    <div className="flex flex-col justify-center items-center">
                      {fixture.score.penalty.away !== null ? (
                        ...
                        <div className="text-sm">(et. ) {fixture.score.extratime.away}</div>
                      ) : null}
                    </div>
                  </div>
          ...
          <button className="absolute right-0 top-1/2 transform -translate-y-1/2 p-2 z-10" onClick={nextItem}>
            <ChevronDoubleRightIcon
              className={`h-10 w-10 ${
                currentIndex === firstItemsFixturesFuture.length - 1 ? "text-gray-600" : "text-neutral-100"
              }`}
            />
          </button>
        </div>
      ...
      <div className="flex flex-col w-full justify-center items-center">
                                ...
                                ${
                                  !fixture.teams.home.winner && !fixture.teams.away.winner
                                    ...
                                    : (fixture.teams.home.winner && fixture.teams.home.id === teamId) ||
                                      (fixture.teams.away.winner && fixture.teams.away.id === teamId)
                                    ? "bg-green-600"
                                    : "bg-red-600"
                                }`}
            ></div>
        ...
        {visibleItemsCount < fixturesByTeamId.length && (
          <div className="p-2">
            <button className="bg-gradient-to-r from-gray-900/80 to-black/60 p-4" onClick={handleShowMore}>
              Show More
            </button>
          </div>
        )}
